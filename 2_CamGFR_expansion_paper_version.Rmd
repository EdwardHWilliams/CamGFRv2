...
Diagnosis_summary <- Expansion_data %>%
  group_by(Diagnosis) %>%
  mutate(Diagnosis = ifelse(Diagnosis == "Unknown" & Patient_type == "Cancer", "Unknown cancer", Diagnosis)) %>%
  mutate(Diagnosis = ifelse(Diagnosis == "Unknown" & Patient_type == "Non-Cancer", "Unknown non-cancer", Diagnosis)) %>%
  summarise(`Number of samples` = n(), `Number of patients` = length(unique(PatientID))) %>%
  arrange(-`Number of samples`)
Diagnosis_summary
write_excel_csv(Diagnosis_summary,
...
Gothenburg_summary <- Expansion_data %>%
  filter(Centre == "Gothenburg") %>%
  dplyr::select("GFR [ml/min]" = GFR, "Creatinine [mg/dL]" = Creat,
...
summary_centre_type <- df_mixed %>%
   group_by(Creatinine_type, Centre) %>%
   summarise(Scr_median = median(Creat))
write_csv(summary_centre_type,
          file.path(fig_export_dir, "Median_Scr_per_centre_type.csv"))
...
Expansion_data %>%
  filter(GFR_index == 1) %>%
  dplyr::select(Creat, Centre, Creatinine_type) %>%
...
Expansion_data %>%
  filter(GFR_index == 1) %>%
  dplyr::select(Creat, Centre, Creatinine_type, Creatinine_method) %>%
...
vars.all <- c("Age", "BSA", "Creat", "GFR", # continuous
              "Sex", "Centre", "Creatinine_type", "Patient_type", "fit_index") # categorical
dat.work <- data.frame(Expansion_data[,vars.all])
dat.work$fit_index <- factor(as.character(dat.work$fit_index), 
                             levels = c("TRUE", "FALSE"), 
                             labels = c("Development", "Validation"))
dat.work$Sex <- factor(dat.work$Sex, levels = c("F", "M"), labels = c("Female", "Male"))
dat.work$Centre <- factor(dat.work$Centre, levels = c("Cambridge", "Manchester", "Gothenburg"))
dat.work$Patient_type[grep("Unknown", dat.work$Patient_type)] <- "Non-Cancer"
dat.work$Patient_type <- factor(dat.work$Patient_type, levels = c("Cancer", "Haem", "Non-Cancer"))
dat.work$Creatinine_type <- factor(dat.work$Creatinine_type)
vars.all.cat <- rep(c(0, 1), c(4, 6))
vars.all.chisq <- rep(c(0, 1), c(4, 6))
...
vars.all.chisq <- vars.all.chisq[-id.rm]
vars.univar <- vars.all[-id.rm]
vars.univar.cat<-vars.all.cat[-id.rm]
marker.units<- marker.units.all[-id.rm]
out1.name <- paste("out1.name", sep="")
assign(out1.name, fsmry.dmgrph(dat = dat.work, vars = vars, vars.cat = vars.cat, vars.chisq = vars.all.chisq, by = "fit_index", all = T, IQR = T, markdown = F))

get("out1.name")[[1]] %>% tibble::remove_rownames() #%>% knitr::kable()
write.csv(get("out1.name")[[1]], 
            paste(file.path(fig_export_dir, "Table_Development_vs_Validation.csv"), sep=""), sep="\t",
          row.names = F)
```
...
summary(Refit_CamGFR)$coef %>%
  ...
  mutate(Coefficient = c("$\\beta_0$", "$\\beta_1$", "$\\beta_2$", "$\\beta_3$", 
                         ...
                         "$\\beta_{11}$", "$\\beta_{12}$")) %>%
  arrange(c(0, 1, 2, 3, 4, 8, 5, 9, 6, 10, 7, 11, 12)) %>% 
  dplyr::select(Variable, Coefficient, everything()) %>%
...
p1 <- Expansion_data_fit %>%
  ...
  mutate(log_Creat = log(Creat)) %>%
  dplyr::select(resid, fitted, Age, log_Creat, BSA, Centre, Creatinine_type) %>%
...
Original_refitted_MAplot <-  Expansion_data %>%
  ...
  dplyr::select(GFR, Original, Refitted) %>%
...
Fitted_df <- Expansion_data %>%
  ...
  mutate(Original = predict(Addenbrooks_sqrt_model, newdata = .)^2) %>%
  mutate(Refitted = predict(Refit_CamGFR, newdata = .)^2) %>%
  dplyr::select(GFR, Original, Refitted, Centre, Creatinine_type) %>% 
...
df <- Expansion_data %>%
  ...
  mutate(Original = predict(Addenbrooks_sqrt_model, newdata = .)^2) %>%
  mutate(Refitted = predict(Refit_CamGFR, newdata = .)^2) %>%
  dplyr::select(GFR, Original, Refitted, Centre, Creat, Creatinine_type,
         Creatinine_method) %>% 
...
#   dplyr::select(Creat, Age, Ht, Wt, BSA, GFR) %>%
#   dplyr::select(GFR, Creat, everything())
...
# cormat <- Expansion_data_fit %>%
#   dplyr::select(Creat, Age, Ht, Wt, BSA, GFR) %>%
#   dplyr::select(GFR, Age, Creat, Urea, Sodium, Albumin, Ht, Wt, BSA) %>%
...
Clean_output_table <- function(table, include_p10_p30 = F){
  t = table %>% 
    ...
    if(include_p10_p30 == T){
      t %>% 
        mutate("P30" = paste0(P30, " (", P30_lwr, ", ", P30_upr, ")")) %>%
...
Results_CreatType_all %>%
  Clean_output_table(include_p10_p30 = T) %>% 
  filter(equation %in% c("Piecewise linear", "Leave-one-out", "CamGFR v2")) %>%
  write_csv(path = file.path(fig_export_dir, "S8_new_models.csv"))
...
Results_CreatType_dose %>% 
  ...
  dplyr::select(Creatinine_type, median, RMSE, IQR, P20_dose, P20_dose_lwr, 
         P20_dose_upr) 
...
df <- Validation_fitted_df %>%
  ...
  filter(Patient_type != "Non-cancer") %>%
  dplyr::select(Creatinine_type, equation, GFR, fitted) %>%
...
mutate(tidy(Refit_CamGFR_Goth), Missing_centre = "Gothenburg") %>%
  ...
  dplyr::select(term, estimate, std.error, Missing_centre) %>% 
