...
cambridge_stat_3methods_tbl <- tableGrob(cambridge_stat_3methods[,c(2:5,8)],
                                         rows=NULL)
...
```{timeline for Cambridge and Manchester with 2 different measurment methods}
...
p_cambridge_distribution_2methods <- ggplot() + 
  ...
  geom_text(aes(label=paste("p", cambridge_stat_2methods$p.format),
                x=2.2, y=1.5), size=label_size, hjust="left") +
  scale_y_continuous(breaks= c(0.3, 0.6, 1.2, 2.4, 4.8), trans = "log10",
                     limits = c(0.3, 4.8)) +
  scale_color_manual(values = method_colors) + 
  labs(y = "SCr",
...
legend_2methods <- cowplot::get_legend(p_cambridge_distribution_2methods)

p_cambridge_2methods <-
  plot_grid(p_cambridge_2methods_timeline, 
...
p_manchester_distribution <- ggplot() + 
  ...
  geom_text(aes(label=paste("p", manchester_stat$p.format),
                x=2.2, y=1.5), size=label_size, hjust="left") +
  scale_y_continuous(breaks= c(0.3, 0.6, 1.2, 2.4, 4.8), trans = "log10",
                     limits = c(0.3, 4.8)) +
  scale_color_manual(values = method_colors) + 
...
p_manchester <-
  plot_grid(p_manchester_timeline, 
            p_manchester_distribution + theme(legend.position = "none"),
            ncol=2,
            rel_widths = c(5,1))

ggsave(plot=pairwise_all,
       filename = "Creatinine_timelines_all_supplement.pdf", 
       path = fig_export_dir,
...
p_jaffe_idms_vs_jaffe_nonidms <- ggplot() + 
  ...
  geom_text(aes(label=paste("p", jaffe_idms_vs_jaffe_nonidms_stat$p.format),
                x=xpos_p, y=1.5), size=label_size, hjust="left") +
  facet_grid(method ~ other, switch="y") +
  coord_trans(x = "log10", xlim = c(exp(-1.8), exp(2.15))) +
  scale_x_continuous(breaks= c(0.3, 0.6, 1.2, 2.4, 4.8)) +
...
p_jaffe_vs_enzymatic <- ggplot() + 
  ...
  geom_text(aes(label=paste("p =", jaffe_vs_enzymatic_stat$p.format),
                x=xpos_p, y=1.5), size=label_size, hjust="left") +
  coord_trans(x = "log10", xlim = c(exp(-1.8), exp(2.15))) +
  scale_x_continuous(breaks= c(0.3, 0.6, 1.2, 2.4, 4.8)) +
  scale_fill_manual(values=method_colors_all) +
...
p_enzymatic_idms_vs_jaffe_nonidms <- ggplot() + 
  ...
  geom_text(aes(label=paste("p", enzymatic_idms_vs_jaffe_nonidms_stat$p.format),
                x=xpos_p, y=1.5), size=label_size, hjust="left") +
  coord_trans(x = "log10", xlim = c(exp(-1.8), exp(2.15))) +
  scale_x_continuous(breaks= c(0.3, 0.6, 1.2, 2.4, 4.8)) +
  scale_fill_manual(values=method_colors_all) +
...
p_idms_jaffe_distribution <- ggplot() + 
  ...
  geom_text(aes(label=paste("p =", idms_jaffe_stat$p.format),
...
p_idms_enzymatic_distribution <- ggplot() + 
  geom_density(data=idms_enzymatic,
               aes(x= Creat, color=Centre),show.legend = showlegends) +
  geom_text(aes(label=paste("p =", idms_enzymatic_stat$p.format),
                x=xpos_p, y=1), size=label_size, hjust="left") +
  coord_trans(x = "log10", xlim = c(exp(-1.8), exp(2.15))) +
  scale_x_continuous(breaks= c(0.3, 0.6, 1.2, 2.4, 4.8)) +
  scale_color_manual(values = centre_colors)  + 
...
p_nonidms_jaffe_distribution <- ggplot() + 
  ...
  geom_text(aes(label=paste("p =", nonidms_jaffe_stat$p.format),
...
ddeeff
"
pairwise_plots <- wrap_plots(a = p_idms_enzymatic_distribution,
                    b = p_jaffe_vs_enzymatic,
                    c = p_idms_jaffe_distribution,
                    d = p_enzymatic_idms_vs_jaffe_nonidms,
                    e = p_jaffe_idms_vs_jaffe_nonidms,
...
```{r, creatinie measures over time and centers}
...
p_cambridge_3methods <- ggplot(data=data_cam, aes(y = (Creat), x = Date_creat)) + 
  geom_point(alpha = 0.1, stroke = 0) + 
  geom_smooth(aes(group = x), colour = "white", method = "gam",
              size = rel(2),
              formula = y ~ s(x, bs = "cs", k=5)) +
  geom_smooth(aes(group = x), colour = "#5b4e9f", method = "gam",
              size = rel(1),
              formula = y ~ s(x, bs = "cs", k=5)) +
  geom_vline(xintercept = as.Date(c("2013-12-12", "2016-03-01")),
             linetype = "dashed") +
...
p_manchester <- ggplot(data=data_manchester,
                       aes(y = (Creat), x = Date_creat)) +
  geom_point(alpha = 0.1, stroke = 0) +
  geom_smooth(
    aes(group = Creatinine_type),
