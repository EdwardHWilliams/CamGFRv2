...
centre_colors <- c(Cambridge = "#5b4e9f", Gothenburg = "#117733",
                   Manchester = "#CC6677")
method_colors <- c("IDMS Enzymatic"="#e6ab02", "IDMS Jaffe"="#d95f02",
...
p_jaffe_idms_vs_jaffe_nonidms <- ggplot(data=jaffe_idms_vs_jaffe_nonidms,
                                        aes(y = x, x = Creat, fill=x)) + 
  geom_violin(position = "dodge", width = 0.9, show.legend = showlegends) + 
...
p_jaffe_vs_enzymatic <- ggplot(data=jaffe_vs_enzymatic,
                           aes(y = x, x = Creat, fill=x)) + 
  geom_violin(position = "dodge", width = 0.9, show.legend = showlegends) + 
  scale_fill_manual(values=method_colors) +
...
p_enzymatic_idms_vs_jaffe_nonidms <- ggplot(data=enzymatic_idms_vs_jaffe_nonidms,
                           aes(y = x, x = Creat, fill=x)) + 
  geom_violin(position = "dodge", width = 0.9, show.legend = showlegends) + 
  scale_fill_manual(values=method_colors) +
  facet_wrap(method ~ ., strip.position = "left") +
...
idms_jaffe <-  df_mixed %>%
  filter(x=="IDMS Jaffe")
p_idms_jaffe_distribution <- ggplot(data=idms_jaffe,
                           aes(x= Creat, color=Centre)) + 
  geom_density(show.legend = showlegends) +
...
p_idms_enzymatic_distribution <- ggplot(data=idms_enzymatic,
                           aes(x= Creat, color=Centre)) + 
  geom_density(show.legend = showlegends) +
  scale_color_manual(values = centre_colors)  + 
...
p_nonidms_jaffe_distribution <- ggplot(data=nonidms_jaffe,
                           aes(x= Creat, color=Centre)) + 
  geom_density(show.legend = showlegends)  +
  scale_color_manual(values = centre_colors) + 
  facet_wrap(. ~ x) +
...
plots <- wrap_plots(a = p_idms_enzymatic_distribution,
           ...
           g = plot_legend,
           design = layout) & theme(plot.margin = unit(c(0,0,0.1,0.1), "cm"))
...
```{r, eval=F}
pm <- ggmatrix(
  list( p_idms_enzymatic_distribution,NULL,NULL,
       p_jaffe_vs_enzymatic,  p_idms_jaffe_distribution,NULL,
       p_enzymatic_idms_vs_jaffe_nonidms, p_jaffe_idms_vs_jaffe_nonidms, p_nonidms_jaffe_distribution),
  nrow = 3, ncol = 3,
  xAxisLabels = c("IDMS Enzymatic", "IDMS Jaffe", "Non-IDMS Jaffe"),
  yAxisLabels = c("IDMS Enzymatic", "IDMS Jaffe", "Non-IDMS Jaffe"),
  showXAxisPlotLabels = FALSE,
  showStrips = TRUE # default
)
pm
```
