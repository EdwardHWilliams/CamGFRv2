...
Expansion_data_fit %>%
  select(Sex, Creatinine_type, log_Creat_nonIDMS_SexF)

set.seed(123)
Refit_CKD <- lm(log_GFR ~ Creatinine_type + BSA + log_Age + 
  ...
  segmented(obj = .,  
            ...
            psi = list("log_Creat_IDMS_SexM" = 0, 
                       "log_Creat_IDMS_SexF" = 0, 
                       "log_Creat_nonIDMS_SexM" = 0, 
                       "log_Creat_nonIDMS_SexF" = 0),
            control = seg.control(tol = 1e-06, it.max = 100, h = 0.1))
Refit_CKD
...
\begin{align*}
GFR &= 
141\times \text{min}\left(\frac{Scr}{\kappa}, 1\right)^{\alpha} \times \text{max}\left(\frac{Scr}{\kappa}, 1\right)^{-1.209} \times Age^{0.993} \times (1+ 0.018Sex_F) \\
  &= \beta_0\times \text{min}\left(\frac{Scr}{\kappa}, 1\right)^{\alpha} \times \text{max}\left(\frac{Scr}{\kappa}, 1\right)^{\gamma} \times Age^{\beta_1} \times (1+ \beta_3Sex_F) \\
\log(GFR)  &= \log(\beta_0) + \alpha\text{min}\left(\log(Scr) - \log(\kappa), 0\right) + \gamma \text{max}\left(\log(Scr) - \log(\kappa), 0\right) \\ 
& \quad + \beta_1\log(Age) \times \log(1+ \beta_3)Sex_F \\
&= \log(\beta_0) + \alpha\log(Scr) - \alpha\log(\kappa) + (\gamma-\alpha)\left(\log(SCr)-\log(\kappa)\right)[\text{if } \log(Scr) > \log(\kappa)] \\
& \quad + \beta_1\log(Age) + \log(1+ \beta_3)Sex_F \\
&= \log(\beta_0) + \alpha_M\log(\kappa_M) + \alpha\log(Scr) + (\gamma-\alpha)\left(\log(SCr)-\log(\kappa)\right)[\text{if } \log(Scr) > \log(\kappa)] \\
& \quad + \beta_1\log(Age) + \left(\log(1+ \beta_3) - \alpha_M\log(\kappa_M) + \alpha_F\log(\kappa_F)\right)Sex_F 
\end{align*}
...
```{r PLOT_spline_CKD, eval = T, echo = F, fig.height=6, fig.width=6, fig.cap="\\label{fig:spline_shape}Shape of Creatine relationship in the CKD-EPI model and our refitted version"}
